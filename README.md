...
3.  Создайте пустое хранилище в этой папке:    появится папка ``.git``, в ней вся история и метаданные.
   ```bash
   git init
   ```
...
17. Добавьте этот файл и сохраните изменения с помощью графической оболчки 
   ```bash
   git gui
   ```
  ...
  - **Дальше можно пользоваться git gui для сохранений.**
